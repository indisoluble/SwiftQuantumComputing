...
public struct LowMemoryCircuitFactory {

    // MARK: - Public init methods

    /// Initialize a `LowMemoryCircuitFactory` instance
    public init() {}
}

// MARK: - CircuitFactory methods

extension LowMemoryCircuitFactory: CircuitFactory {

    /// Check `LowMemoryCircuitFactory.makeCircuit(gates:)`
    public func makeCircuit(gates: [Gate]) -> Circuit {
        ...
        let elementTransformation = CircuitMatrixElementStatevectorTransformation(matrixFactory: matrixFactory)
        let directTransformation = DirectStatevectorTransformation(transformation: elementTransformation)
