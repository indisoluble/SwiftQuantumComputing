...
class FixedBitFlipNoiseTests: XCTestCase {

    // MARK: - Tests

    func testTwoIdenticalNoises_equal_returnTrue() {
        // Given
        let noise = FixedBitFlipNoise(probability: 0.1, target: 1)
    ...
    func testTwoNoisesWithSameProbabilitiesButDifferentTargets_equal_returnFalse() {
        // Given
        let oneNoise = FixedBitFlipNoise(probability: 0.1, target: 1)
        let anotherNoise = FixedBitFlipNoise(probability: 0.1, target: 2)
