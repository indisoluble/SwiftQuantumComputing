...
class OracleXGateTests: XCTestCase {
    ...
    func testZeroTruthTableQubits_init_throwException() {
        ...
        // Then
        XCTAssertThrowsError(try OracleXGate(truthTable: [], truthTableQubitCount: 0, gate: gate))
    }
    ...
    func testGateThatThrowsException_makeFixed_throwException() {
        ...
        let factory = try! OracleXGate(truthTable: [], truthTableQubitCount: 1, gate: gate)
    ...
    func testValidGateAndLessInputsThanNecessary_makeFixed_throwException() {
        ...
        let factory = try! OracleXGate(truthTable: [],
    ...
    func testValidGateAndEnoughInputs_makeFixed_returnExpectedGate() {
        ...
        // Then
        switch factory.makeFixed(inputs: inputs) {
        case .success(.oracleX(let truthTable, let controls, let gate)):
    ...
    func testComposedGateAndEnoughInputs_makeFixed_returnExpectedGate() {
        // Given
        let gate = ConfigurableGateTestDouble()
        gate.makeFixedResult = .oracleX(truthTable: ["0"],
                                        controls: [3, 1, 4],
                                        gate: .not(target: 0))

        let factory = try! OracleXGate(truthTable: ["1"], truthTableQubitCount: 2, gate: gate)
        ...
        case .success(.oracleX(let truthTable, let controls, let gate)):
            ...
            XCTAssertEqual(gate,
                           .oracleX(truthTable: ["0"], controls: [3, 1, 4], gate: .not(target: 0)))
        default:
            XCTAssert(false)
        }
    }
