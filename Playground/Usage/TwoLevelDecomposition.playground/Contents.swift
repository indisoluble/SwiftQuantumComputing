...
try circuitFactory.makeCircuit(gates: gates).statevector().get()
...
let decomposition = try solver.decomposeGates(gates).get()
...
try drawer.drawCircuit(decomposition).get()
...
start = CFAbsoluteTimeGetCurrent()
try circuitFactory.makeCircuit(gates: decomposition).statevector().get()
diff = CFAbsoluteTimeGetCurrent() - start
